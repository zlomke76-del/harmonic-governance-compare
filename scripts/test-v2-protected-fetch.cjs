...
must('Vercel Protection Bypass for Automation can answer the first request with');
must('res.status === 307 && bypassSecret');
must('res.headers.get("set-cookie")');
must('setCookie.split(";")[0]');
must('Cookie: cookie');
must('unresolved redirect HTTP');
